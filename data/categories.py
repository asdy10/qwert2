...
def get_names_link_categories():
    ...
    for cat in avto_moto:
        ...
        full_names_array.append([{'Запчасти и автотовары': avto_moto[cat]}, {cat: params['category']}])
        params = {}
...
def get_arr_cat1(full_names_array):
    arr = []
    for i in full_names_array:
        #print(i)
        if [j for j in i[0].keys()][0] not in arr:
            arr.append([j for j in i[0].keys()][0])
    return arr
